      ...
      <div className={`fixed inset-0 z-[80] backdrop-blur-3xl transition-all duration-700 ${adminOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'} ${isDarkMode ? 'bg-[#0a0a0a]/98' : 'bg-[#e8e7e7]/98'}`}>
        <div className="flex flex-col items-center justify-center min-h-screen p-8 relative">
          ...
          <div className="max-w-md w-full space-y-6">
            <h2 className="text-2xl font-black uppercase tracking-widest text-center mb-10">Admin Control</h2>
            <div className="space-y-4">
              <input type="text" placeholder="Title" value={newEp.title} onChange={e => setNewEp({...newEp, title: e.target.value})} className="w-full bg-black/5 dark:bg-white/5 p-4 rounded-xl outline-none border border-transparent focus:border-[#f28d35]" />
              <div className="flex gap-4">
                <input type="text" placeholder="Duration" value={newEp.duration} onChange={e => setNewEp({...newEp, duration: e.target.value})} className="flex-1 bg-black/5 dark:bg-white/5 p-4 rounded-xl outline-none border border-transparent focus:border-[#f28d35]" />
              ...
              <input type="text" placeholder="Audio URL / File Name" value={newEp.fileId} onChange={e => setNewEp({...newEp, fileId: e.target.value})} className="w-full bg-black/5 dark:bg-white/5 p-4 rounded-xl outline-none border border-transparent focus:border-[#f28d35]" />
              <textarea placeholder="Description (One line per bullet)" rows={4} value={newEp.content} onChange={e => setNewEp({...newEp, content: e.target.value})} className="w-full bg-black/5 dark:bg-white/5 p-4 rounded-xl outline-none border border-transparent focus:border-[#f28d35]" />
            ...
            <div className="max-h-40 overflow-y-auto space-y-2 custom-scrollbar pr-2">
              {episodes.map(ep => (
                <div key={ep.id} className="flex items-center justify-between p-3 border-b border-black/5 dark:border-white/5">
                  ...
                  <button onClick={() => handleDeleteEpisode(ep.id)} className="text-red-500 opacity-40 hover:opacity-100 transition-opacity"><Trash2 size={16} /></button>
                </div>
              ))}
            </div>
          </div>
